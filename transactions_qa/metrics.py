...
def bleu(predictions, targets) -> dict:
    """Computes bleu score."""
    print("predictions generated for bleu score", predictions[:5])
    print("targets generated for bleu score", targets[:5])
    print("shape bleu", len(targets))
...
def avg_f1_score_with_invalid(predictions, targets) -> dict:
    label_list = ["0", "1"]
    targets, predictions = np.asarray(targets), np.asarray(predictions)
    # Get indices of invalid predictions.
    invalid_idx_mask = ~np.isin(predictions, label_list)
    print("Mask", invalid_idx_mask)
