...
@dataclass
class MeanHourDiffTaskOpenEnded(NumericTaskAbstract):
    """
    A task for floating-point Open-Ended QA task: given a continuous numeric target - hour_diff,
    answer question with a numeric answer.
    """

    def __post_init__(self):
        self.task_name = "mean_hour_diff_open-ended"
        self.target_feature_name = 'hour_diff'  # [0, 8000+] range of values, but crop to [0, 95]

        self.task_special_token = None
        self.task_specific_special_token = "[mean_hour_diff_openended]"
        ...
        self.ending_prompts = [
            ". What is the average difference in hours between two subsequent customer's transactions?"
...
@dataclass
class LastHourDiffTaskOpenEnded(NumericTaskAbstract):
    """
    A task for floating-point open-ended QA task: given a continuous numeric target - hour_diff,
    answer question with a numeric answer.
    """
    ...
    def __post_init__(self):
        self.task_name = "last_hour_diff_open-ended"
        self.target_feature_name = 'hour_diff'  # [0, 8000+] range of values, cropped to [0, 95]

        self.task_special_token = None
        self.task_specific_special_token = "[last_hour_diff_openended]"
        ...
        self.ending_prompts = [
            ". What is the difference in hours between two last customer's transactions?"
            " Answer a number from the range from 0 to 95."
            " In case the answer is larger then 95, answer 95 as maximum significant difference."
        ]
