...
class MostFrequentDateTaskOpenEnded(CategoricalTaskAbstract):
    ...
    def __post_init__(self):
        ...
        self.task_specific_special_token = "[most_freq_date_openended]"
...
@dataclass
class LastDateTaskOpenEnded(CategoricalTaskAbstract):

    def __post_init__(self):
        self.task_name = "last_date_open-ended"
        self.target_feature_name = 'date'

        self.task_special_token = None
        self.task_specific_special_token = "[last_date_openended]"
        ...
        self.ending_prompts = [
            ". What was the date of the last client's transaction?"
            " Answer a date in DD/MM format, where DD - is a day and MM - is a month."
            " For example 31 April is a 31/04."
        ]
    ...
    def process_input_batch(self, batch: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        for i, (dow_feature_, woy_feature_, mask_) in enumerate(zip(day_of_week_feature_batch,
            ...
            joined_feature_masked = torch.cat([dow_feature_masked.unsqueeze(0),
                                               woy_feature_masked.unsqueeze(0)], dim=0)
            last_feature = joined_feature_masked.permute(-1, 0)[-1]  # get a single Tensor value of a feature
