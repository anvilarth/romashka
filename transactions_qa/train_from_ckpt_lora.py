...
def main():
    ...
    if last_checkpoint is not None:
        ...
        if "epoch" in list(ckpt.keys()):
            logger.info(f"Epoch: {ckpt['epoch']}, step: {ckpt['global_step']}")
        if "hyper_parameters" in list(ckpt.keys()):
        ...
        for key in ckpt['state_dict'].keys():
            if key.startswith("model"):
                ...
                model_state_dict[model_key] = ckpt['state_dict'][key]
    ...
    if last_checkpoint is not None:
        logger.info(f"Initialize with checkpoint weights....")
        # Init with checkpoint weights
        curr_model_state_dict = model_.state_dict()
        resized_loading_state_dict = collections.OrderedDict()

        # encoder_only_state_dict
        for param_name, param in model_state_dict.items():  # _renamed
            curr_param = curr_model_state_dict.get(param_name, torch.empty([1, ]))
            if param.size() != curr_param.size():
                print(f"Param with name {param_name} doesn't match size: {param.size()} vs. {curr_param.size()}!")
                new_param, fill_param = (param.clone(), curr_param) \
                    if param.size(0) > curr_param.size(0) else (curr_param.clone(), param)
                if len(fill_param.size()) > 1:
                    new_param[:fill_param.size(0), :fill_param.size(1)] = fill_param
                else:
                    new_param[:fill_param.size(0)] = fill_param

                resized_loading_state_dict[param_name] = new_param
            else:
                resized_loading_state_dict[param_name] = param

        encoder_only_state_dict_renamed = collections.OrderedDict()
        # keys_to_rename = {'language_model.lm_head.0.weight': 'language_model.lm_head.weight'}
        keys_to_rename = {'language_model.lm_head.weight': 'language_model.lm_head.0.weight'}

        # encoder_only_state_dict resized_loading_state_dict model_state_dict
        for k, val in resized_loading_state_dict.items():
            if k in list(keys_to_rename.keys()):
                encoder_only_state_dict_renamed[keys_to_rename.get(k)] = val
            else:
                encoder_only_state_dict_renamed[k] = val

        # Load state dict
        out = model_.load_state_dict(
            # resized_loading_state_dict,
            # model_state_dict,
            encoder_only_state_dict_renamed,
            strict=False)
        logger.info(f"Checkpoint initialization results:\n{out}")
