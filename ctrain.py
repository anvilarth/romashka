...
for epoch in range(num_epochs):
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        cls_token = model.cls_token.repeat(batch_size, 1, 1)
        original = torch.cat([cls_token, original], dim=1)
        ...
        corrupted = mixup_data(original, mask=mask)
        corrupted_features = model.encoder(corrupted, mask=mask.unsqueeze(1).unsqueeze(2))

        aug_features_1 = model.mlp(original_features[:, 0])
        aug_features_2 = model.mlp(corrupted_features[:, 0])

        aug_features_1 = (aug_features_1 / aug_features_1.norm(dim=-1, keepdim=True))
        aug_features_2 = (aug_features_2 / aug_features_2.norm(dim=-1, keepdim=True))
